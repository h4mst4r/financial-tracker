...
| # | Action | Detail |
|---|---|---|
| **H1** | **Retire the design bible as the visual arbiter** | Stop treating `_bmad-output/.../design-bible/` as the source of truth — the ux-spec's named values are. Keep the bible only as an optional *generated* reference, or remove it. |
| **H2** | **Repoint / delete `design-bible-parity.test.ts`** | Replace the bible↔`index.css` diff with a direct **spec↔`index.css` token-parity** test (L1/L1a/L17). `/design-system` stays as the L18 completeness **gallery** (every primitive by its real component), **not** a pixel-diff. |
| **H3** | **Update CLAUDE.md P5 (+ P1)** | P5 "spec defines → bible renders → app matches bible" → "spec defines concrete values → tests enforce → `/design-system` demos." Drop the "diff against the rendered prototype" clause in P1/DoD; keep the `/design-system` completeness gallery. |
...
> **Swept clean (no action — recorded so effort isn't wasted here):** arbitrary-Tailwind values are only the 4 B10 hits; `scrollbar-gutter`/themed scrollbar **is** wired (`index.css`, 12 sites); motion **durations are tokenized** (`duration-quick`/`duration-press` + 37 `--duration`/`--ease`/`motion-factor` sites); `DisplayCurrencyPicker`/`ViewContextSwitcher` **reuse** `Dropdown` (not bespoke); `EmptyState` is built and consumed; `onClick`-on-`div` is a11y-handled (B11), not raw. The build's real debt is **B1 (value atoms)** + **B8 (behaviors)** + **B12 (derive the ramp)** — three focused builds, not scattered magic.
