...
- **Columns:** checkbox · Date · Name(+method/desc sub-line) · Payee(`Avatar`) · Category(`Badge`, colour leads) · Currency(`Badge`) · Amount(`MonetaryValue` columnar+signColour) · Base SGD(`MonetaryValue` columnar+signColour) · status(`StatusBadge` dot) · ⋮. All data columns sortable; shared=default (personal icon-flagged); tags = small colour `Badge`s after Category.
