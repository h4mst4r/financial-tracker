...
Signs colour the **figure's text** (amount/sign), not status — **all status colouring is the Status registry below** (transaction `paid/pending/cancelled` etc. resolve there, single source). Cross-currency uses the arrow form. A **semantic hue feeds the §3 resolver** to tint a container (alerts, zones). A **semantic** `Badge` = `{semantic}-fill` (the semantic colour at `alpha-fill`, §6) + solid `{semantic}` text; the **§5 entity** family instead tints by entity colour (`bg-entity-fill-calm` + `text-entity-fg`, §153) and the **neutral/outline** family carries no hue (`surface-active` / bordered). **Law:** no green/red/amber hex in TSX.

**Status registry — the one status binding.** Every status indicator (chip · freshness badge · ledger `dot` · alert badge) is a `Badge` (the dissolved `StatusBadge` — §397) driven by a **tone** from a central registry; a surface passes a *domain + status key*, **never a colour**. **Five tones — each *is* a §0/§4 token, so they reskin per theme:** `positive` = `success` (green) · `warning` = `warning` (amber) · `critical` = `error` (red) · `neutral` = grey (`surface-active`/`border`) · `info` = `info` (blue — the §0 semantic anchor, **not** `accent-secondary`). **No domain uses a sixth tone**, and each domain picks a *subset* (every current domain uses ≤ 4). **Registry (authoritative — add a status = one row):**
...
| Transaction (`dot`) | pending → warning · completed → positive · reconciled → positive · cancelled → neutral (`reconciled` is foreign-currency only — a base-currency row has no FX to verify; it is success-green like `completed`, distinguished by label + the unreconciled-worklist filter) |
...
**Form submission & required-field validation (every `EntityModal` / form, app-wide).** The primary action (Save/Add) is **never disabled for missing or invalid required fields** — it disables **only** while a submit mutation is in-flight (so it can't double-fire). On a submit attempt with unmet required / invalid fields: **(1)** each offending `Field` shows `error-ring` (§6) and the **primary action** (Save/Add) shakes once (`error-bounce`, §13) to signal "not allowed" (the shake is on the *button*, not the fields); **(2)** focus moves to the **first** offending field; **(3)** a **`Zone`/Info-box** with the **`error`** semantic tint (§4→§3) renders **above the footer** carrying the summary note ("Please complete the required fields."). After the first attempt, fields **re-validate on change** so a ring clears the moment its field is fixed. Live format errors (bad amount / unparseable date) still use the inline `Field` error (§6) as-you-type. This **supersedes** any per-field "Save disabled" gating (e.g. `RecurringDateInput` keeps its inline parse error but no longer disables Save). **Law:** a disabled primary action used as the *only* signal for missing required input FAILS.
...
**Page gutter (one source).** The routed-content gutter is owned **once** by **AppShell `main`** via a named `--page-gutter` token (`lg` 24px ≥ md · `md` 16px < md); **routed pages never add their own padding** — so a new route can't ship edge-to-edge (the recurring miss). A surface that needs full-bleed (e.g. an immersive Viewer) opts out **explicitly**, it is not the default. **Law:** a page component that hand-rolls its own outer `p-*` gutter FAILS.
...
| **Transaction flags** (`TRANSACTION_FLAG_ICON` — typed behavioural flags, ARCH §3.7; **not** tags) | personal (non-shared outflow)→`UserRound` · gst_claimable→`Receipt` |
...
| error-bounce | emphatic 500 | spring | translate (shake) — the **primary action** on a blocked submit (§6) | none |
...
| **Badge** ✓ | `<span>` (+ Icon, + opt Dot) — *absorbs StatusBadge + FilledChip* | §4 status / §5 entity / neutral `surface-active` / outline (non-semantic, §328) · §2 · §8 |
...
| **ConfirmationDialog** ✓ | Modal + message + Button(s) + **optional confirm-input slot** (a *safeguard* on high-risk destructive actions, e.g. type-to-confirm — still a decision, not a form) + **optional tertiary action** — the ONE sanctioned 3-way decision: the **duplicate-detection resolver** (*Save anyway* / *Link as duplicate* / *Cancel*, FR-E-008, §12); every other dialog stays 2-action | confirm = `error` (§4) **only when destructive**, else primary · tertiary (Link) = `outline`, Cancel = `ghost` |
...
| **EntityPage** ✓ | Toolbar (⊇ FilterBar module) + card grid (EntityCard) / Table + EmptyState + Skeleton | §17 card-grid (auto-fit) · §18 loading/empty |
| **Toolbar** ○ | the generic module top-strip (**one wrapping row**): **identity** (name H3 §2 + info-text) · **actions** (sort Pressable · grid/list `SegmentedControl` view-toggle · `+New` Button; + grid-end "+New" ghost tile) · **`FilterBar` filter module** (when the page filters) | §17 → FilterBar Filters-popover collapse `< md` |
| **EntityCard** ✓ | Card + Avatar/Icon + MonetaryValue(hero) + MiniSparkline + FavouriteStar + ContextMenu + Badge(archived) + owner Avatars (**always shown when the entity has any owner — incl. a single owner**, so ownership is always legible) | §3 entity calm/vivid fill · §6 selection ring/fill + tick · §13 hover-lift |
...
| **FilterBar** ✓ | the Toolbar's **filter module** — descriptor controls (`search │ dateRange │ dropdown │ popover`; type filters use `dropdown`, **not** `segmented`) + **`{count} ×`** clear (shared, §"the page bars"); **two profiles** (record-list · aggregation) | §1 surface · serialises to `VisualizationFilter` |
...
- **decisions** → `ConfirmationDialog` (incl. destructive + the optional safeguard-input + the one 3-action variant — the duplicate-detection resolver, §12).
...
### Toolbar (⊇ FilterBar) · BulkActionBar — the page bars

The **`Toolbar`** is the generic top-strip for every module route — **one wrapping row**: **identity** (name · info) · **actions** (sort · view-toggle · +New) · a **`FilterBar` filter module** (present when the page filters). `FilterBar` stays a reusable descriptor primitive, but at the page level it is **always rendered inside the Toolbar — never a bare sibling**; the Viewer composes it in its own control bar. **`BulkActionBar` is a separate bar** (different trigger × target — the selection, not the view) and is reused without the Toolbar (e.g. `CategoryTree`); it shares only the clear micro-pattern below.

| bar | trigger | target | proof it composes / stands |
|---|---|---|---|
| **Toolbar** | always | the collection / view (identity · actions · **FilterBar module**) | every module route |
| **FilterBar** (Toolbar module) | when the page filters | the **query** → serialises to `VisualizationFilter` | the **Viewer** composes it in its own control bar |
| **BulkActionBar** | only on **≥1 selected** (slides up, §13 bulk-bar-slide) | the **selection** | **CategoryTree** (not an EntityPage) uses it |

- **Filters live in the FilterBar module of the Toolbar.** The Toolbar composes the FilterBar (one wrapping row; the module collapses to a Filters popover `< md`, §17). No page renders a bare FilterBar beside the toolbar.
- **Clear affordance (shared).** Both the FilterBar module and the BulkActionBar show **`{count} ×`** — the active count immediately followed by a trailing `×` icon-button (`ACTION_ICON.close`, §11). FilterBar's count = active filters (this replaces the old "Clear all" text); BulkActionBar's count = selected rows. Same micro-pattern, so "how many, and how to clear" reads identically on both.
- **SegmentedControl is for mode / tab switching, not filtering** — `TabView`, the grid/list view-toggle, density, Household↔Individual. A **type filter is a `Dropdown`** (a query select); it is never a `SegmentedControl` (which reads as tabs).
- **BulkActionBar actions** ∈ { **Button** · **inline picker** (`Dropdown` — a single-target pick: Categories Edit-type / Move / Merge) · **destructive** → `ConfirmationDialog` }. The bar owns its parameterised pickers **inline** — there is **no separate "bulk chooser" modal**. A **multi-field** bulk edit (events "Edit shared fields") is a plain `EntityModal`.
...
| **Alerts** ○ | AppShell + `Toolbar` (Mark-all-read; ⊇ FilterBar module: type · unread/all) + date-grouped `AlertPanel` rows — reached from the topbar bell, not a sidebar module | Alerts |
...
> 2. **Header** = the generic `Toolbar` in the **fixed order** (name H3 · info-text · sort · grid/list · show-archived · the **`FilterBar` module** [search · entity filters] · `+New`) — one wrapping row, never re-ordered or bespoke. A computed page with no creation (Debt) simply omits `+New`; it does **not** restyle the bar.
...
`AppShell` + `EntityPage⟨Account⟩`. **Four sidebar routes mount the *same* page, filtered by `subtypes`** (`/accounts` = bank+credit_card · `/capital` · `/assets` · `/insurance`); the three single-subtype routes **lock the Type Dropdown** (the `EntityModal`) to that subtype. No bespoke per-subtype page. Toolbar: name + info ("5 accounts · S$ 22,370 net") + Sort + grid/list + **+New account**, with the FilterBar module carrying the **bank/credit type filter** (a `Dropdown`, §"the page bars"). Grid of `EntityCard` (calm/vivid · hero `MonetaryValue` · `MiniSparkline` · `FavouriteStar` · owner `Avatar`s) ending in a "+New" ghost tile. Card variants: asset · capital (ROI) · insurance (coverage) · bank (interest) · credit-card (debt-leading). Tap → `EntityDetailView`; ⋮→Edit → `EntityModal`.
...
### Transactions
`AppShell` + `Table` (RecordLedger). **Toolbar**: name + info (count · out/in base) + **Archived `Toggle`** + **+New**, with its **`FilterBar` module** (record-list): search · date range · category · **type (`Dropdown`)** · Filters popover (account/person/status/GST/tags/reconciled) → `VisualizationFilter`. The **Archived toggle** (same affordance as Accounts/Categories) surfaces archived transactions (`include_archived`) so a row can be reached for ⋮ Restore.
- **Columns:** checkbox · Date · Name(+method/desc sub-line) · Payee(`Avatar`) · Category(`Badge`, colour leads) · Currency(`Badge`) · Amount(`MonetaryValue` columnar+signColour) · Base SGD(`MonetaryValue` columnar+signColour) · status(`Badge` status+dot) · ⋮. All data columns sortable. **Behavioural flags** render as **muted `Icon`s clustered after the Name cell** (aria-labelled — meaning via glyph, never colour alone, §11/L15): a **non-shared (personal) outflow** → `UserRound` (shared is the quiet default → no flag), a **GST-claimable** row → `Receipt`; both appear on the desktop row and the `< md` card. **Tags** = small colour `Badge`s after Category — a **distinct system** (`MultiSelectField`, Stories 5.9–5.10); the two flags are **typed booleans, never tags** (ARCH §3.7), which is why they are icons, not `Badge`s.
...
- **Transaction modal** (`EntityModal` + money block): **account field** — its label switches by direction (**outflow → "Paid with" · inflow → "Paid to"**); **selectable = `bank` + `credit_card` accounts + Cash only** (capital/asset/insurance are snapshot-valued and funded via **Transfers** (Transfers module / Story 6.5), never a transaction leg — only `bank`/`credit_card` are ledger-backed). The account picker is a **searchable `Dropdown`** whose rows are **account-type glyph + name + institution (`muted`) + owner `Avatar`(s)**, so same-name accounts and subtypes disambiguate; account FX-formula auto-fill / Cash spot. **Category** picker = searchable `Dropdown`, each option = **leading category-colour `Dot` + the option name tinted to that category's colour** (the `Dot` carries the identity and the name stays a legible non-colour signal; the tint resolves through the **entity-axis floor** — `--entity-colour` → `text-entity` (§2/§3a), the same floor-safe, vivid-aware path `CategoryTree` uses — **never raw hex**, so it holds contrast on every theme). · Amount · Base (auto, read-only-until-override; source **border tone via the §4 FX-base-source registry** — formula→`info`/spot→`neutral`/manual→`warning` (§10 border) + spot·fee·Δ) · Flags (outflow only: Shared-default-on · GST) · **Tags** (`MultiSelectField` create-on) · duplicate-detection on save (see the Duplicate-detection bullet below).
- **Duplicate detection (on save — modal Save · inline commit · quick-add):** before a create/edit persists, the entered tuple (household · `amount` ±0.01 · `event_date` ±2 days · `category_id` · `transaction_type` · `payee_person_id`) is checked (`GET /api/events/duplicate-check`, **self-excluded** on edit via `exclude_id`). On a hit the **`ConfirmationDialog` 3-action duplicate variant** opens: the matched candidate(s) as read rows (name · `DateValue` · `MonetaryValue` · category `Dot`), **select-one** radios when >1 (**most-recent pre-selected**), and **Save anyway** (filled primary) / **Link as duplicate** (`outline` — sets `duplicate_of` to the **selected** candidate) / **Cancel** (`ghost` — aborts; an inline commit reverts the cell). `duplicate_of` is **never auto-resolved** — only the explicit Link sets it (FR-E-008, ARCH §4.10). No hit → the save proceeds unchanged. One shared detection helper serves all three save surfaces (quick-add's pinned row is Story 5.7, which reuses the helper).
...
- **Occurrence history** (expand): timeline + `Badge` (status) (upcoming/processed/skipped/missed/failed); processed → clickable linked transaction (cross-module); missed = red. Per-occurrence: Skip · Trigger now · Process now.
...
- **Budget card:** glyph `Badge` (entity, icon-only) + name + period `Badge` + ⋮ · **limit vs actual** (`MonetaryValue`) · **`ProgressBar` by health** (green<thr / amber≥`alert_threshold_pct` / red>100%) · "S$ N left/over" + budget-health `Badge` (status) · drill ("N transactions →"). Actuals computed live; **parent budget = whole-subtree total** (subcategory rollup).
...
- **Toolbar** (no `+New` — alerts aren't user-created): name "Alerts" + info ("{n} unread") + a **Mark-all-read** `Button` in the primary-action slot, with its **`FilterBar` module** (record-list): **type filter** `Dropdown` (All + the 7 `alert_type`s) · **unread / all** `Dropdown` (a filter, not a mode-switch — §"the page bars").
