...
### Transactions
`AppShell` + `Table` (RecordLedger). **Toolbar**: name + info (count · out/in base) + **+New**, with its **`FilterBar` module** (record-list): search · date range · category · **type (`Dropdown`)** · Filters popover (account/person/status/GST/tags/reconciled) → `VisualizationFilter`.
