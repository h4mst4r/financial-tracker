...
- `Button.tsx` — confirms **B10** (`active:scale-[0.97]`), **B14** (`disabled:opacity-50`); variant naming **drifts** from the spec (`primary/secondary` vs spec `filled/outline`; `text/link/icon` variants not built). **✅ variant set closed by 5f-9** — `primary→filled`/`secondary→outline` value-preserving rename done across every `Button` call site + the new `text`/`link`/`icon` variants built (`icon` = bare, size-to-child, not a control-height square) + demoed on `/design-system` (L18). The raw-`<button>` consumer migration onto these + the L8 `<button>`→`Button` eslint promotion → **5.12** (the eslint row stays commented). *(B10 press-scale itself was the `@utility press-scale` promotion, closed by 5f-7.)*
