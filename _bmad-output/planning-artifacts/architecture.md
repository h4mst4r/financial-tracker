...
| `SERVICE_URL` | config | The service's own URL — the OIDC `audience` `get_job_auth` verifies for `/jobs/*` (§5.6). Unset locally (the shared-bearer `SERVICE_ACCOUNT_KEY` path carries dev/manual triggers); set in prod where OIDC is required. |
...
- **Circuit breaker:** when **every** enabled provider fails for a currency (non-200 / missing or
  ...
  `/jobs/fx-refresh` (Story 3.7) is the **first `alerts` writer** — it raises `FX_API_DOWN` via the
  shared `create_alert` helper (the streak is derived from `last_rate_at` staleness, no counter
  column), deduped against an undismissed alert. Epic 10's `/jobs/alerts` + read/ack API +
  `AlertPanel` reuse that same helper.
- **Freshness:** update `last_rate_at`; **stale at > 48 h → `FX_RATE_STALE`** — generated by
  `/jobs/alerts` (Epic 10), which reads the `last_rate_at` that `/jobs/fx-refresh` maintains
  (3.7 maintains the freshness data; it does **not** itself raise `FX_RATE_STALE`) (UX §10).
